...
const Transactions = () => {
    ...
    const [transactionsList, setTransactionsList] = useState([
        { id: 1, category: 'expense', name: 'Groceries', date: 'Jan 29', amount: -100, icon: groceriesIcon, customise: false },
        { id: 2, category: 'expense', name: 'Restaurant', date: 'Jan 28', amount: -50, icon: restaurantIcon, customise: false },
        { id: 3, category: 'expense', name: 'Gas', date: 'Jan 27', amount: -40, icon: gasIcon, customise: false },
        { id: 4, category: 'income', name: 'Paycheck', date: 'Jan 26', amount: 5000, icon: Wallet, customise: false }
    ]);
    ...
    const handleCustomise = (id) => {
        setTransactionsList(prevList => {
            return prevList.map(transaction => {
                return transaction.id === id ? { ...transaction, customise: !transaction.customise } : transaction
            })
        })
    }
    ...
    return (
        <>
            ...
            <main className='p-4 sm:ml-[80px]'>
                ...
                <h1 className='md:text-[22px] sm:text-[18px] font-bold mb-12'>Transactions</h1>
                ...
                {transactionsList.map(transaction => {
                    return (
                        <article key={transaction.id} className={`flex mt:5 sm:mt-[10px] md:mt-[20px] ${category === 'all' || transaction.name === category ? null : 'hidden'} bg-white flex-wrap md:flex-row ${transaction.category === 'expense' && transactionType === 'income' ? 'hidden' : null} ${transaction.category === 'income' && transactionType === 'expense' ? 'hidden' : null} justify-between h-[90px] justify-between py-2 rounded-xl px-5`} >
                            {transaction.customise === false ? <div className='grid grid-cols-4 w-[200px]'>
                                <div className='cols-start-1 place-content-center cols-span-1'>
                                    <img className='m-auto bg-gray-200 rounded-md p-2' src={transaction.icon} alt={transaction.name} />
                                </div>
                            ...
                            </div> : <div className='flex flex-row gap-7 mx-auto my-auto'>
                                <button onClick={() => {
                            ...
                            <div className='md:flex sm:hidden flex-row gap-7 ml-auto my-auto'>
                            ...
                            <div className='relative sm:flex md:hidden flex-row gap-7 sm:order-2 md:order-last ml-auto my-auto'>
                                <button onClick={() => handleCustomise(transaction.id)}><FontAwesomeIcon icon={faEllipsisVertical} /></button>
                            </div>
