        ...
        <aside className={`z-40 bg-white h-screen transition-all rounded-[10px] overflow-hidden fixed duration-1000ms ease-in ${open ? 'w-[220px]' : 'w-[75px]'}`}>
            <nav className='h-full overflow-hidden sticky flex flex-col space-y-3 pt-5 px-5 border-r shadow-sm'>
                <div>
                    ...
                    {open ? 
                        <img src="./images/angle-left-solid.svg" className='w-[17px] h-[30px] mb-[30px] cursor-pointer' alt="open menu" onClick={() => { setOpen(!open) }}></img> 
                        : <img src="./images/angle-right-solid.svg" className='w-[17px] h-[30px] mx-auto mb-[30px] cursor-pointer' alt="close menu" onClick={() => { setOpen(!open) }}></img>}
